...
```{python}
data_dir = '/home/ndettmer/data/soltani_distance/1_Distance_train'
data_dir2 = '/home/ndettmer/data/soltani_distance/2_Distance_train2'
...
## Manually check for correct labels
...
```{python}
mixed_dir = '/home/ndettmer/data/distance_regression/'
ds = ImageFolder(mixed_dir)
distances = pd.read_csv(os.path.join(mixed_dir, 'mixed/distances.csv'))
idxs = np.array(range(len(ds)))
...
idx = idxs[i]
img, _ = ds[idx]
filename, _ = ds.samples[idx]
filename = os.path.basename(filename)
dist = distances[distances.Image == filename].iloc[0].Distance
...
<div style="color:green"><b>Labels seem to be correct!</b></div>


## Check value range of images

```{python}
from torchvision.transforms import ToTensor
ds = ImageFolder(mixed_dir, transform=ToTensor())
samples = np.random.choice(idxs, size=10)
```
