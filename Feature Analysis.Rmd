...
pred_df = pd.read_csv(f'/home/ndettmer/experiments/ma/classification/maize_ternary_manual_preselect/predictions/inat/{model_version}/23-05-18_12-40_predictions.csv')
```

```{python}
pred_df[pred_df.target_text == 'soil'].prediction.unique()
```

```{python}
pred_df[(pred_df.target_text == 'soil') & (pred_df.prediction_text == 'soil')]
```
...
```{python}
maize_sample_pid = get_safe_pred_sample('maize').pid.values[0]
maize_sample, _, _ = dm.ds.dataset.get_item_by_pid(maize_sample_pid)

weed_sample_pid = get_safe_pred_sample('weeds').pid.values[0]
