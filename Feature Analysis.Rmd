...
```{python}
x, *_ = dm.ds.dataset.get_item_by_pid('207105943')
```
...
attributions_ig_nt = noise_tunnel.attribute(input, target=pred_label_idx, n_steps=200, nt_samples=10, nt_type='smoothgrad_sq')
```
...
pred_df[(pred_df['prob_std_zscore'] > 3) & (pred_df['prob_std'] < prob_std_mean)].sort_values('prob_std', ascending=False)
```

```{python}
pred_df[(pred_df.target == 1) & (pred_df.prediction == 1)].nlargest(5, columns=['prob_std'])
```
