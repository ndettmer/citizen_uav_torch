...
```{python}
from CitizenUAV.math_utils import gram_matrix, channel_mean_std
...
```{python}
means, stds = channel_mean_std(ds)
```
...
```{python}
# TODO: Is this normal?!
norm = Normalize(list(means.numpy()), list(stds.numpy()))
plt.imshow(to_pil_image(norm(img).mean(dim=0)))
```
...
```{python}
sds = Subset(ds, range(len(ds)))
```

```{python}
sds.dataset.dataset.transform
```

```{python}
ds.dataset.transform.transforms.append(norm)
plt.imshow(to_pil_image(ds[0][0]))
```

```{python}
sum(['Normalize' in str(t) for t in ds.dataset.transform.transforms]) == 1
```
