...
```{python}
# TODO: create confusion matrix for labelled area
```
...
```{python}
# Finding maize predictions
# get samples for each class out of the raster dataset

rnd_idx = list(range(len(june_ds)))
random.shuffle(rnd_idx)
i = 0
pred_sample_size = 9

maize_predicted_samples = []
maize_confs = []
maize_predicted_idx = []
while len(maize_predicted_samples) < pred_sample_size:
    img, t = june_ds[rnd_idx[i]]
    ...
    if train_classes[pred] == 'zea mays':
        maize_predicted_samples.append(pil_img)
        maize_confs.append(conf)
        maize_predicted_idx.append(rnd_idx[i])
    
    i += 1
```

```{python}
maize_pred_side_len = np.sqrt(pred_sample_size)
figure, axs = plt.subplots(maize_pred_side_len, maize_pred_side_len, figsize=(20, 20))
for i, pil_img in enumerate(maize_predicted_samples):
    conf = maize_confs[i]
...
## Playing around with prediction maps
<!-- #endregion -->
...
```{python}
prob_maps = np.load('/home/ndettmer/experiments/ma/classification/maize_ternary/predictions/220518_maize_rgb/23-01-24_15-58_probability_map')
```

```{python}
# WARNING: Execute with caution, can lead to memory overflow!

driver = 'ESRI Shapefile'
vector_file = '/home/ndettmer/experiments/ma/classification/maize_ternary/predictions/220518_maize_rgb/23-01-24_10-19.shp'
for label, species in enumerate(classes):
    gen = ({'properties': {'raster_val': v, 'species': species}, 'geometry': s} for i, (s, v) in enumerate(shapes(ds.rds.read(1), mask=bool_pred_masks[species], transform=ds.rds.transform)))
    with fiona.open(vector_file, 'w', driver=driver, crs=str(ds.rds.crs), schema={'properties': [('raster_val', 'int')], 'geometry': 'Polygon'}) as dst:
        dst.writerecords(gen)
    del gen
```

```{python}
# extremely slow! maybe use another file format!

with open('/home/ndettmer/experiments/ma/classification/maize_ternary/predictions/220518_maize_rgb/23-01-24_17-03_probability_map.yml', 'r') as infile:
