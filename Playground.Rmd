...
```{python}
gt_ds = GTiffDataset(filename, shape_dir, stride=10)
```
...
for j, i in enumerate(c):
    ...
    img = to_pil_image(gt_ds.get_bb_data(bb))
    axs[j].imshow(img)
plt.show()
```
...
img[:, (gt_ds.get_mask_bool() & ~gt_ds.labelled_area)] = gray_img[(gt_ds.get_mask_bool() & ~gt_ds.labelled_area)]
del gray_img
img = to_pil_image(torch.from_numpy(img))
...
for i in c:
    bb = gt_ds.bbs[i]
    x_min, x_max, y_min, y_max = bb
    txt = ax.text(y_min - 5, x_min - 5, f"{x_min}, {x_max}, {y_min}, {y_max}", color='white')
