...
```{python}
from CitizenUAV.data import GTiffDataset
ds = GTiffDataset('/home/ndettmer/ssd_data/UOS_Data/220613_maize_rgb_test_area.tif', '/home/ndettmer/ssd_data/UOS_Data/1306_maize', stride=32)
```

```{python}
from matplotlib import pyplot as plt
plt.imshow(ds.soil_mask)
```

```{python}
plt.imshow(ds.labeled_area_cropped)
```

```{python}
min_point_geo = ds.labeled_area_transform * (0, 0)
min_point_ds = ~ds.rds.transform * min_point_geo
min_point_ds = tuple(reversed(min_point_ds))

soil_mask_cropped = ds.soil_mask[int(min_point_ds[0]):int(min_point_ds[0]) + ds.labeled_area_cropped.shape[0], int(min_point_ds[1]):int(min_point_ds[1]) + ds.labeled_area_cropped.shape[1]]
```

```{python}
plt.imshow(soil_mask_cropped)
```

```{python}
soil_mask_cropped.shape, ds.labeled_area_cropped.shape
```
...
```{python}
ds.labeled_area_cropped.shape
```

```{python}
from copy import deepcopy
deepcopy(ds.labeled_area_transform)
```

## pixel_conf_mat debug
...
train_data_dir = '/home/ndettmer/ssd_data/UOS_Data/maize_ternary_valds_mixed'
shape_dir = '/home/ndettmer/ssd_data/UOS_Data/1805_maize'
dataset_path = '/home/ndettmer/ssd_data/UOS_Data/220518_maize_rgb_test_area.tif'
window_size = 128
stride = 16
class_map = None
pred_file = '/home/ndettmer/experiments/ma/classification/maize_ternary_raster_training_data/predictions/220518_maize_rgb_test_area/version_0/23-04-24_11-04_probability_map.npy'
...
```{python}
~(target_map[inat_cls_idx]).sum()
```

```{python}
target_map[inat_class_to_idx['soil']].sum()
```

```{python}
from matplotlib import pyplot as plt
plt.imshow(target_map == 1)
```
...
```{python}
inat_class_to_idx['soil']
```

```{python}
from CitizenUAV.models import InatMogaNetClassifier, InatSequentialClassifier
import torch
```

```{python}
seq = InatSequentialClassifier(n_classes=3, backbone_model='resnet18', lr=.0001, weight_decay=.001)
```

```{python}
mog = InatMogaNetClassifier(n_classes=3, model_size='small', lr=.0001, weight_decay=.001)
```
...
```{python}
y_seq = seq(x)
y_seq.min(), y_seq.mean(), y_seq.max()
```

```{python}
y_mog = mog(x)
y_mog.min(), y_mog.mean(), y_mog.max()
```
...
```{python}
mog = InatMogaNetClassifier.load_from_checkpoint('/home/ndettmer/experiments/ma/classification/maize_ternary_manual_preselect/lightning_logs/version_6/checkpoints/epoch=49-step=43200.ckpt')
```
