...
from RevCol.modules import ConvNextBlock, Decoder, LayerNorm, SimDecoder, UpSampleConvnext
from RevCol.revcol_function import ReverseFunction
...
class FullNet(nn.Module):
    def __init__(self, channels=[32, 64, 96, 128], layers=[2, 3, 6, 3], num_subnet=5, kernel_size=3, num_classes=1000,
        ...
        if not inter_supv:
            ...
            self.cls_blocks = nn.ModuleList(
                [Classifier(in_channels=channels[-1], num_classes=num_classes) for _ in range(4)])
            if num_classes <= 1000:
                channels.reverse()
                self.decoder_blocks = nn.ModuleList(
                    [Decoder(depth=[1, 1, 1, 1], dim=channels, block_type=ConvNextBlock, kernel_size=3) for _ in
                     range(3)])
            else:
                self.decoder_blocks = nn.ModuleList(
                    [SimDecoder(in_channel=channels[-1], encoder_stride=32) for _ in range(3)])
...
def revcol_tiny(save_memory=False, inter_supv=True, drop_path=0.1, num_classes=1000, kernel_size=3, **kwargs):
    channels = [64, 128, 256, 512]
    layers = [2, 2, 4, 2]
    num_subnet = 4
    return FullNet(channels, layers, num_subnet, num_classes=num_classes, drop_path=drop_path, save_memory=save_memory,
                   inter_supv=inter_supv, kernel_size=kernel_size)


##-------------------------------------- Small -----------------------------------------

def revcol_small(save_memory=False, inter_supv=True, drop_path=0.3, num_classes=1000, kernel_size=3):
    channels = [64, 128, 256, 512]
    layers = [2, 2, 4, 2]
    num_subnet = 8
    return FullNet(channels, layers, num_subnet, num_classes=num_classes, drop_path=drop_path, save_memory=save_memory,
...
def revcol_base(save_memory=False, inter_supv=True, drop_path=0.4, num_classes=1000, kernel_size=3, head_init_scale=None):
    channels = [72, 144, 288, 576]
    layers = [1, 1, 3, 2]
    num_subnet = 16
    return FullNet(channels, layers, num_subnet, num_classes=num_classes, drop_path=drop_path, save_memory=save_memory,
                   inter_supv=inter_supv, head_init_scale=head_init_scale, kernel_size=kernel_size)


##-------------------------------------- Large -----------------------------------------

def revcol_large(save_memory=True, inter_supv=True, drop_path=0.5, num_classes=1000, kernel_size=3, head_init_scale=None):
...
##--------------------------------------Extra-Large -----------------------------------------
def revcol_xlarge(save_memory=True, inter_supv=True, drop_path=0.5, num_classes=1000, kernel_size=3, head_init_scale=None):
