...
from citizenuav.processes import pixel_conf_mat, predict_geotiff
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--n_supplement_min",
        type=int,
        help="Maximum number of raster supplement samples per class.",
        required=False,
        default=0,
    )
    parser.add_argument("--n_supplement_max", type=int, help="Maximum number of raster supplement samples per class.")
    parser.add_argument(
        "--n_supplement_step", type=int, help="Stipe size for increasing number of raster supplement samples per class."
    )
    ...
    f1_df = pd.DataFrame(
        columns=["version", "n_supplement", "May F1", "May Weed Precision", "June F1", "June Weed Precision"],
        dtype=float,
    )
    ...
    for n_supplement in range(args.n_supplement_min, args.n_supplement_max, args.n_supplement_step):
        ...
        for i_run in range(args.n_runs):
            ...
            if args.patience > 0:
                callbacks.append(
                    EarlyStopping(
                        monitor="val_cce", mode="min", patience=args.patience, verbose=True, min_delta=args.min_delta
                    )
                )
    ...
    notification.notify(title="Raster Share Evaluation", message="Done.")
