...
```{python}
data_dir = '/home/ndettmer/data/distance_regression/mixed'
model_path = '/home/ndettmer/experiments/ma/distance_regression/lightning_logs/version_1/checkpoints/epoch=15-step=3248.ckpt'
seed = 42
...
```{python}
df.to_csv('/home/ndettmer/experiments/ma/distance_regression/22-10-17_eval.csv')
```
...
for idx, row in tqdm(df.sample(frac=.2).iterrows()):
    ...
    img = Image.open(os.path.join(data_dir, row.filename))
    plt.imshow(img)
    plt.savefig(f'/home/ndettmer/experiments/ma/distance_regression/eval_img/{idx}.png')
    plt.close()
```
